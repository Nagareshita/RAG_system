...
def main():
    ...
    # LLM selection via factory (local HF or API-backed)
    if cfg.generation.llm_provider == "local_hf":
        if not cfg.generation.llm_model_path:
            raise RuntimeError("generation.llm_model_path must be set for local_hf provider")
        ...
        raise RuntimeError(f"Unsupported llm_provider: {cfg.generation.llm_provider}")
