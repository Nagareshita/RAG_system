# Copied and adapted from legacy extractor, stripped of UI coupling.
...
from ast_jsonl_validator.modelica_parser_loader import new_parser
from ast_jsonl_validator.modelica_models import (
    SymbolRecord,
    CodeSpan,
    EquationBlock,
    Parameter,
...
from ast_jsonl_validator.utils_text import file_bytes, sha1_text
...
def extract_dir(directory_path: Path, progress_callback=None) -> List[SymbolRecord]:
    ...
    if progress_callback:
        progress_callback("ファイルをスキャン中...", 0, 1)
    ...
    total_files = len(mo_files)
    if total_files == 0:
        if progress_callback:
            progress_callback("Modelicaファイルが見つかりません", 0, 1)
        return records
    
    if progress_callback:
        progress_callback(f"{total_files}個のファイルを処理中...", 0, total_files)
    
    # 各ファイルを処理
    for index, file_path in enumerate(mo_files, 1):
        try:
            ...
            if progress_callback:
                progress_callback(
                    f"処理中: {file_path.name} ({index}/{total_files})",
                    index,
                    total_files
                )
            ...
            if progress_callback:
                progress_callback(
                    f"エラー: {file_path.name} ({index}/{total_files})",
                    index,
                    total_files
                )
    
    if progress_callback:
        progress_callback(f"抽出完了: {len(records)}個のシンボル", total_files, total_files)
