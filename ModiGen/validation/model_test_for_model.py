...
from modigen.config import load_config
import re
...
def load_libraries(lib_files, lib_files_dir, version, omc):
    cfg = load_config()
    # Optionally set additional library search paths
    if cfg.omc.extra_library_paths:
        joined = os.pathsep.join(cfg.omc.extra_library_paths)
        omc.sendExpression(f'setModelicaPath("{joined}")')

    if cfg.omc.use_loadModel:
        omc.sendExpression(f'loadModel(Modelica, {{"{version}"}})')
    else:
        template = cfg.omc.modelica_stdlib_package_template
        if not template:
            raise RuntimeError("Modelica stdlib template not set and use_loadModel is False.")
        stdlib_path = template.format(version=version)
        omc.sendExpression(f'loadFile("{stdlib_path}")')
